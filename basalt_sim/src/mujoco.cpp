...
public:
  MujocoNode() : Node("mujoco_node") {
    ...
    std::cout << "Loaded worldbody" << std::endl;
    std::cout.flush();
    
    std::cout << "Loaded world" << std::endl;
    std::cout.flush();

    std::cout << "Loaded xmls" << std::endl;
    std::cout.flush();
    ...
    for (int i = 0; i < num_drones; i++) {
      ...
      // Per drone type specs
      auto x500_spec = mj_parseXML("mujoco/x500_standalone.xml", nullptr, nullptr, 0);

      // Create drone spec 
      mjs_attach(worldbody->element, x500_spec->element, drone_name.c_str(), "");
