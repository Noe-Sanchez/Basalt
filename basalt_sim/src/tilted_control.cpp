...
class TiltedController : public rclcpp::Node{
  public:
    TiltedController(): Node("tilted_node"){
      // Namespace for tf
      this->declare_parameter<std::string>("tf_namespace", "fah_hexa_1");

      // Subscribers;
      sim_pose_subscriber     = this->create_subscription<nav_msgs::msg::Odometry>("/model/fah_hexa_1/odometry",    10, std::bind(&TiltedController::sim_pose_callback,     this, std::placeholders::_1));
      desired_pose_subscriber = this->create_subscription<nav_msgs::msg::Odometry>("/control_1/reference/pose", 10, std::bind(&TiltedController::desired_pose_callback, this, std::placeholders::_1));

      // Temporal bool for diff
      kill_switch_subscriber = this->create_subscription<std_msgs::msg::Bool>("/control_1/kill_switch", 10, std::bind(&TiltedController::kill_switch_callback, this, std::placeholders::_1));

      // Publishers
      control_f_publisher = this->create_publisher<geometry_msgs::msg::Wrench>("/control_1/control_force", 10);
      motor_publisher     = this->create_publisher<actuator_msgs::msg::Actuators>("/fah_hexa_1/command/motor_speed", 10);
      ...
      //control_timer   = this->create_wall_timer(10ms, std::bind(&TiltedController::control_callback, this));
      control_timer   = this->create_wall_timer(50ms, std::bind(&TiltedController::control_callback, this));
      ...
      // Srry for messy code lol
      double phi = deg2rad(20.0);

      double alpha1 = deg2rad(30.0);
      double alpha2 = deg2rad(-30.0);
      double alpha3 = deg2rad(-90.0);
      double alpha4 = deg2rad(-150.0);
      double alpha5 = deg2rad(150.0);
      double alpha6 = deg2rad(90.0);
      double C_phi = std::cos(phi);
      double S_phi = std::sin(phi);

      double C_a1 = std::cos(alpha1), S_a1 = std::sin(alpha1);
      double C_a2 = std::cos(alpha2), S_a2 = std::sin(alpha2);
      double C_a3 = std::cos(alpha3), S_a3 = std::sin(alpha3);
      double C_a4 = std::cos(alpha4), S_a4 = std::sin(alpha4);
      double C_a5 = std::cos(alpha5), S_a5 = std::sin(alpha5);
      double C_a6 = std::cos(alpha6), S_a6 = std::sin(alpha6);

      double M11 = -kT * C_a1 * S_phi;
      double M21 =  kT * S_a1 * S_phi;
      double M31 =  kT * C_phi;
      double M41 =  C_a1 * (l * kT * C_phi - kQ * S_phi);
      double M51 =  S_a1 * (-l * kT * C_phi + kQ * S_phi);
      double M61 =  l * kT * S_phi + kQ * C_phi;

      double M12 =  kT * C_a2 * S_phi;
      double M22 = -kT * S_a2 * S_phi;
      double M32 =  kT * C_phi;
      double M42 =  C_a2 * (l * kT * C_phi - kQ * S_phi);
      double M52 =  S_a2 * (-l * kT * C_phi + kQ * S_phi);
      double M62 = -l * kT * S_phi - kQ * C_phi;

      double M13 =  kT * C_a3 * S_phi;
      double M23 =  kT * S_a3 * S_phi;
      double M33 =  kT * C_phi;
      double M43 =  C_a3 * (l * kT * C_phi + kQ * S_phi); 
      double M53 =  S_a3 * (-l * kT * C_phi + kQ * S_phi);
      double M63 =  l * kT * S_phi + kQ * C_phi;

      double M14 =  kT * C_a4 * S_phi;
      double M24 = -kT * S_a4 * S_phi;
      double M34 =  kT * C_phi;
      double M44 =  C_a4 * (l * kT * C_phi - kQ * S_phi);
      double M54 =  S_a4 * (-l * kT * C_phi + kQ * S_phi);
      double M64 = -l * kT * S_phi - kQ * C_phi;

      double M15 = -kT * C_a5 * S_phi;
      double M25 =  kT * S_a5 * S_phi;
      double M35 =  kT * C_phi;
      double M45 =  C_a5 * (l * kT * C_phi - kQ * S_phi);
      double M55 =  S_a5 * (-l * kT * C_phi + kQ * S_phi);
      double M65 =  l * kT * S_phi + kQ * C_phi;

      double M16 =  kT * C_a6 * S_phi;
      double M26 = -kT * S_a6 * S_phi;
      double M36 =  kT * C_phi;
      double M46 =  C_a6 * (l * kT * C_phi + kQ * S_phi);
      double M56 =  S_a6 * (-l * kT * C_phi + kQ * S_phi);
      double M66 = -l * kT * S_phi - kQ * C_phi;
      ...
      // Populate M_phi with its pseudoinverse
      M_phi = M_phi.completeOrthogonalDecomposition().pseudoInverse();

      // Print matrix to ensure no nans
	...
	//uaux_lin(i) = -k1(i) * pow(abs(sigma(i)), 0.5) * sign(sigma(i)) - k2(i) * sigma(i);
      ...
      //fu(0) = 0;
      //fu(1) = 0;
      fu(2) = std::max(-m*2.5,  std::min(m*2.5, fu(2)));
	...
	e_ang = 2 * e_ang.normalized() * acosaux;
      ...
      std::cout << "uaux_ang: " << uaux_ang.transpose() << std::endl;
      ...
      motor_speeds = M_phi*flat_outputs;

      for (int i = 0; i < 6; i++) {
	if (motor_speeds(i) > 0.0) {
	  motor_speeds(i) = sqrt(motor_speeds(i));
	} else if (motor_speeds(i) < 0.0 || std::isnan(motor_speeds(i))) {
	  motor_speeds(i) = 0.0;
        }
      }
	...
	motor_speed.velocity[4] = std::clamp(motor_speeds(4), 0.0, 2000.0);
	motor_speed.velocity[5] = std::clamp(motor_speeds(5), 0.0, 2000.0);*/
	
	motor_speed.velocity[0] = std::clamp(motor_speeds(5), 0.0, 2000.0);
	motor_speed.velocity[1] = std::clamp(motor_speeds(0), 0.0, 2000.0);
	motor_speed.velocity[2] = std::clamp(motor_speeds(1), 0.0, 2000.0);
	motor_speed.velocity[3] = std::clamp(motor_speeds(2), 0.0, 2000.0);
	motor_speed.velocity[4] = std::clamp(motor_speeds(3), 0.0, 2000.0);
	motor_speed.velocity[5] = std::clamp(motor_speeds(4), 0.0, 2000.0);
  ...
  private:
    ...
    Eigen::Matrix<double, 6, 6> M_phi; // Fah matrix
...
int main(int argc, char * argv[]){
  ...
  rclcpp::spin(std::make_shared<TiltedController>());
  rclcpp::shutdown();
  return 0;
}
