...
def generate_launch_description():
  ...
  bridge_config_path = os.path.join(get_package_share_directory("basalt_sim"), "config", "fah_config.yaml")
  ...
  for i in range(num_drones):
    control_node = Node(
      package="basalt_sim",
      executable="tilted_node",
      name=f"tilted_node_{i+1}",
      output="screen",
      remappings=[("/control_1/reference/pose",   f"/control_{i+1}/reference/pose"),
                  ("/model/fah_hexa_1/odometry",  f"/model/fah_hexa_{i+1}/odometry"),
                  ("/control_1/control_force",    f"/control_{i+1}/control_force"),
                  ("/control_1/error",            f"/control_{i+1}/error"),
                  ("/fah_hexa_1/command/motor_speed", f"/fah_hexa_{i+1}/command/motor_speed"),],
    )
    ...
    with open(bridge_config_path, "a") as f:
      f.write(f'- topic_name:     "/fah_hexa_{i+1}/command/motor_speed"\n  ros_type_name:  "actuator_msgs/msg/Actuators"\n  gz_type_name:   "gz.msgs.Actuators"\n  direction:      ROS_TO_GZ\n\n- topic_name:     "/model/fah_hexa_{i+1}/odometry"\n  ros_type_name:  "nav_msgs/msg/Odometry"\n  gz_type_name:   "gz.msgs.Odometry"\n  direction:      GZ_TO_ROS\n\n')

  ros_gz_bridge_node = Node(package="ros_gz_bridge",
                            executable="parameter_bridge",
                            name="ros_gz_bridge",
                            output="screen",
                            parameters=[{"config_file": os.path.join(get_package_share_directory("basalt_sim"), "config", "fah_config.yaml")}],
                           )
